...
if (!address) {
    console.error("Need to give IP address as args")
}

const backendHost = import.meta.env.MODE === "development" ? "http://localhost:1337" : `http://${import.meta.env.VITE_IP_ADDRESS}`
...
const getRefreshedToken = async () => {
    try {
        const result = await axios.get(`${backendHost}/refresh`, { withCredentials: true })
        return result.data
    } catch (error: any) {
